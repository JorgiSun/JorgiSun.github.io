**⚖️ Terms of Service**

These Terms of Service (“Terms”) govern your use of the Discord bot “Crazy Mobile” (“the Bot”). By using the Bot, you agree to these Terms.

---

### 1. Acceptance of Terms

By interacting with the Bot, you agree to comply with and be bound by these Terms.
If you do not agree, you must not use the Bot.

---

### 2. Bot Usage

The Bot is provided “as is” and may be modified, suspended, or discontinued at any time without notice.
You may use the Bot only for lawful purposes and in accordance with Discord’s Terms of Service and Community Guidelines.

---

### 3. User Accounts

The Bot uses your Discord account to identify you and store in-bot data, including:

* User IDs
* Usernames
* Game-related progression data (such as in-game currency, claimed players, and evolutions)

You are responsible for maintaining the confidentiality of your account and for all activities under your account.

---

### 4. Restrictions

You may not:

* Use the Bot for any illegal activity
* Attempt to interfere with the Bot’s operation
* Collect data from other users without consent
* Abuse any system, exploit bugs, or manipulate gameplay in unintended ways

---

### 5. Data and Privacy

The Bot stores minimal data required to function. See the **Privacy Policy** for detailed information.
The Bot does not sell, share, or distribute your data outside the Bot.

---

### 6. Limitation of Liability

The developer is not liable for:

* Any loss of data or in-game progress
* Any indirect, incidental, or consequential damages
* Errors or downtime of the Bot

---

### 7. Modifications to Terms

The Bot developer may update these Terms at any time. Continued use of the Bot after changes constitutes acceptance of the updated Terms.

---

### 8. Contact

For questions about these Terms, please contact the Bot developer via:

* Discord: jorgidesigns
* Official server: [https://discord.com/invite/Rz8hezZFSC](https://discord.com/invite/Rz8hezZFSC)
